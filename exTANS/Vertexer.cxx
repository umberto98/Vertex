...
double Vertexer::FindVertexZ(TClonesArray* hits1, TClonesArray* hits2, TH1F* evzdist){
	...
	vector <double> closez;
	for(int k=0; k<tracksz.size();k++)
		if (TMath::Abs(tracksz[k]-bincentre)<fZStep) closez.push_back(tracksz[k]);
	double vertz=0;
	for (int l=0; l<closez.size(); l++) vertz+=closez[l];
	vertz=vertz/closez.size();
	evzdist->Reset();
	return vertz;
}
